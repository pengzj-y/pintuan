...
| cutRange | 切量范围 | 0=全部放行, 1-99=按用户ID哈希取模放行对应比例 |
| scBlacklist | 渠道黑名单 | 如 s02c02 |
| cacheSwitch | 缓存开关 | 0=开启, 1=关闭 |

**切量算法：**

```
hash(userId) % 100 <= cutRange → 放行
```

例如 `cutRange=60` 表示 60% 的用户能通过，40% 被拦截。

**特性：**
- 更新后**立即生效**：同时更新本地配置和 Redis Topic
- 集群同步：通过 Redis pub/sub 通知所有实例同步更新
- 无需重启服务
...
| 用户ID | 昵称 | 用途 |
|--------|------|------|
| `xfg01` ~ `xfg09` | 测试用户 | 预置测试账号（部分已参与活动） |
| `xfg_new` | 新用户 | 全新账号，可测试所有活动 |
| `xfg_test1` ~ `xfg_test5` | 测试账号1~5 | 全新测试账号 |

### 商品与活动

| 商品ID | 商品名称 | 原价 | 活动ID | 优惠类型 | 实付 |
|--------|----------|------|--------|----------|------|
| `9890001` | 《手写MyBatis：渐进式源码实践》 | ¥100 | 100123 | 直减 ¥20 | ¥80 |
| `9890002` | 《Redis实战与源码分析》 | ¥128 | 100124 | 直减 ¥30 | ¥98 |
| `9890003` | 《SpringBoot微服务架构实战》 | ¥199 | 100125 | 满200减50 | - |
| `9890004` | 《DDD领域驱动设计》 | ¥99 | 100126 | N元购 ¥69 | ¥69 |
| `9890005` | 《Netty网络编程实战》 | ¥168 | 100127 | 满150减40 | - |
| `9890006` | 《深入理解JVM虚拟机》 | ¥158 | 100128 | 直减 ¥25 | ¥133 |
...
│   ├── views/
│   │   ├── CustomerView.vue ← C端拼团商城（类似拼多多，含账号切换）
...
| 页面 | 路由 | 功能 |
|------|------|------|
| 拼团商城 | `/customer` | C端用户页面（类似拼多多），支持商品浏览、参团、开团 |
...
前端提供**账号切换**功能，位于侧边栏底部和C端页面顶部，无需后端接口：

| 功能 | 说明 |
|------|------|
| **切换账号** | 点击侧边栏"账号切换"展开面板，点击任意用户一键切换 |
| **查看当前用户** | 面板顶部显示当前用户ID和昵称 |
| **自定义添加** | 可输入用户ID和昵称，添加自定义测试账号 |
| **持久化** | 用户列表和当前用户保存在 `localStorage`，刷新页面不丢失 |
| **恢复默认** | 点击"恢复默认账号"可重置为预置账号列表 |

**预置账号列表：**

```
xfg01 ~ xfg09   普通测试账号（部分已参与活动 100123）
xfg_new         全新账号（未参与任何活动）
xfg_test1 ~ 5   全新测试账号
```
